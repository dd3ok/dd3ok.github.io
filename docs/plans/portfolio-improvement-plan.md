...
- 2026-07-01: Started `agent-skills` catalog update on branch `codex/agent-skills-catalog-update`; added regression coverage for flat skill ordering, home back link, and expanded public skill repositories before updating the page UI and metadata.
- 2026-07-01: Pruned duplicate and out-of-scope `/agent-skills` entries by removing New Session Handoff in favor of Savepoint and keeping only NaverStock/TossInvest among securities API skills; verified with `npm run build`.
